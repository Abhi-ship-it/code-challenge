...
class CompaniesControllerTest < ApplicationSystemTestCase
  ...
  test "Show action without city and state" do
  ...
  test "Show action with city and state" do
  ...
  test "Update with invalid email address" do
    ...
    within("form#edit_company_#{@company.id}") do
      fill_in("company_name", with: "Updated Test Company")
      fill_in("company_zip_code", with: "93009")
      fill_in("company_email", with: "updated_test_company@test.com")
      click_button "Update Company"
    end

    assert_text "Email must be a getmainstreet.com account"
  end
  ...
  test "destroy cancel the window alert" do
    ...
    assert_equal a.text, "Are you sure you want to permanently delete this company?"
    a.dismiss
